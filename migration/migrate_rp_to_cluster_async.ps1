...
param(
    ...
    [parameter(Mandatory=$false)] $target_ds_cluster
...
$SSD_STORAGE_PODS = @("ssd-mm02-cld01", "sp-mm01-cld08", "sp-mm02-cld05", "sp-mm02-cld06", "sp-sj01-cld04")
if (!$customer_id){
...
if($target_ds_cluster){
    $script:TARGET_DS_CLUSTER = $target_ds_cluster
}else{
...
if ( (check_free_space $vms $TARGET_DS_CLUSTER) -ne $true ) {
    write-host "Insufficient space in target datastore cluster"
    exit
}
